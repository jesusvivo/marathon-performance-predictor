...
- **Phase 10 (CI + reproducible ingestion/deploy)**: GitHub Actions CI plus a scripted refresh-and-ship loop.
  - **CI** (`.github/workflows/ci.yaml`): ruff + mypy + pytest on every push/PR (free on a public repo). Data-dependent steps stay local: the Garmin data is private and gitignored, so public CI never touches it.
  - **`make ingest`**: idempotent rebuild from the current export (features -> ONNX refit + parity -> Feast materialize). **`make deploy`** (`scripts/deploy.sh`): bento build -> Cloud Build (amd64, server-side) -> Cloud Run redeploy. Together, `make ingest && make deploy` refreshes and ships.
  - Ingestion cadence is a deliberate compliance choice: Garmin's full GDPR export is heavyweight and occasional, and the unofficial Garmin API was ruled out on TOS grounds, so refresh is occasional-full-export, which matches how slowly fitness state evolves. An incremental path via the Garmin Connect activities CSV (lighter, more frequent) is **designed but deferred**: the CSV lacks Garmin's training load (derivable via TRIMP from HR), race flags, and wellness, so it is a future enhancement rather than a blocker.
  - Also added Pydantic request/response models on the service (typed `RacePrediction`, input validation), and slimmed the README so Swagger is the single source of truth for the API contract.
